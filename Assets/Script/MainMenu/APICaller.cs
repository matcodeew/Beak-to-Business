...
public static class APICaller
{
    ...
    private static string api_url = "http://b2b.zapto.org/api/api.php?endpoint=";
    ...
    public static async Task<string> GetUserSkinById(int id)
    {
        return await GetUserSkinByIdRequest(id);
    }
    ...
    private static async Task<string> GetUserSkinByIdRequest(int _id)
    {
        using (UnityWebRequest request = UnityWebRequest.Get(api_url + "skins/" + _id))
        {
            var operation = request.SendWebRequest();
            ...
            {
                string jsonResponse = request.downloadHandler.text;
                return JsonHelper.FromSingleJson<string>(jsonResponse);
            }
        }
    }
}
